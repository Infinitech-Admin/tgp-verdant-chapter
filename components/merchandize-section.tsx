  ...
  return (
    <section className="py-12 px-4 sm:px-6 lg:px-8 bg-[#f2faf2] min-h-screen relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="bg-white rounded-3xl shadow-sm p-4 mb-6 border border-[#d0e8d0]">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div className="md:col-span-5">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 border border-[#c8e6c8] rounded-full focus:outline-none focus:ring-2 focus:ring-[#2d7a2d]/40 focus:border-[#2d7a2d] text-[#1a2e1a] placeholder-[#6b8f6b]"
                />
              </div>
            </div>
            ...
            <div className="md:col-span-3">
              <select
                ...
                className="w-full px-4 py-2 border border-[#c8e6c8] rounded-full focus:outline-none focus:ring-2 focus:ring-[#2d7a2d]/40 focus:border-[#2d7a2d] text-[#1a2e1a]"
              >
                {categories.map((cat) => (
        ...
        {loading ? (
          ...
          <>
            ...
            {pagination.last_page > 1 && (
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white rounded-3xl shadow-sm p-4 border border-[#d0e8d0]">
                          ...
                          <button
                            ...
                            className={`w-10 h-10 rounded-full text-sm font-medium transition-colors ${
                              pagination.current_page === pageNum
                                ...
                                : "border border-[#c8e6c8] hover:bg-[#e8f5e8] text-[#1a2e1a]"
                            }`}
                          >
                            {pageNum}
                          </button>
...
function ProductCard({
  ...
  const router = useRouter(); // add this inside ProductCard component
  ...
  const handleAddToCart = async () => {
    ...
    // ✅ check BEFORE setAdding so button doesn't get stuck
    if (!token) {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-3xl shadow-sm hover:shadow-lg hover:shadow-[#1a4d1a]/10 transition-all duration-300 overflow-hidden group relative border border-[#d0e8d0] hover:border-[#2d7a2d]"
    >
      {/* Success Indicator */}
      {showSuccess && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 bg-[#1a4d1a] border-2 border-[#d4a017] text-[#d4a017] rounded-full p-4 shadow-xl"
        >
      ...
      <div className="relative w-full h-48 bg-[#e8f5e8] overflow-hidden">
        ...
        {/* Stock Badge */}
        {product.stock === 0 && (
          <div className="h-[20px] absolute top-2 right-2 bg-[#7a2020] text-white text-xs font-bold px-2 py-1 rounded-full">
            Out of Stock
          </div>
        )}
        {product.stock > 0 && product.stock < 10 && (
          <div className="absolute top-2 right-2 bg-[#d4a017] text-white text-xs font-bold px-2 py-1 rounded-full">
            Low Stock
          </div>
        )}
        {inCart && (
          <div className="absolute top-2 left-2 bg-[#1a4d1a] text-white text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1">
        ...
        {/* Category */}
        {product.category && (
          <span className="absolute top-2 right-2 px-3 py-1 bg-[#2d7a2d] text-white text-xs font-medium rounded-full mb-2">
            {product.category}
          </span>
        )}
      </div>
      ...
      <div className="p-4">
        {/* Name */}
        <h3 className="text-lg font-bold text-[#1a2e1a] mb-1 line-clamp-2 group-hover:text-[#1a4d1a] transition-colors truncate">
          {product.name}
        </h3>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm text-[#b8870c] font-medium">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-end gap-2">
              <button
                ...
                disabled={adding}
                className="p-1 border border-[#c8e6c8] rounded-full hover:bg-[#e8f5e8] disabled:opacity-50"
              >
              ...
              <input
                ...
                disabled={adding}
                className="w-16 text-center border border-[#c8e6c8] rounded-full py-1 text-[#1a2e1a] disabled:opacity-50"
              />
              <button
                onClick={() =>
            ...
            <button
              ...
              disabled={adding}
              className="w-full py-2 bg-[#1a4d1a] text-white rounded-full hover:bg-[#2d7a2d] disabled:bg-gray-400 transition-all font-semibold flex items-center justify-center gap-2 hover:scale-[1.02]"
            >
              {adding ? (
                <>
